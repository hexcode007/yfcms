...
class Response extends AbstractMessage implements ResponseInterface {
    ...
    $statusCode, array $headers = [], StreamInterface $body = null, array $options = []
    ) {
    ...
    public function json(array $config = []) {
        try {
            return Utils::jsonDecode(
                            (string) $this->getBody(), isset($config['object']) ? !$config['object'] : true, 512, isset($config['big_int_strings']) ? JSON_BIGINT_AS_STRING : 0
            );
    ...
    public function xml(array $config = []) {
        ...
        try {
            ...
            $xml = new \SimpleXMLElement(
                    (string) $this->getBody() ? : '<root />', isset($config['libxml_options']) ? $config['libxml_options'] : LIBXML_NONET, false, isset($config['ns']) ? $config['ns'] : '', isset($config['ns_is_prefix']) ? $config['ns_is_prefix'] : false
            );
            ...
            'Unable to parse response body into XML: ' . $e->getMessage(), $this, $e, (libxml_get_last_error()) ? : null
            );
        }
