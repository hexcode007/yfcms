...
class MessageFactory implements MessageFactoryInterface {
    ...
    $statusCode, array $headers = [], $body = null, array $options = []
    ) {
        if (null !== $body) {
    ...
    public function createRequest($method, $url, array $options = []) {
        ...
        if ($method == 'POST' && !isset($options['body']) && !isset($options['json'])
        ) {
            $options['body'] = [];
        }
    ...
    public function fromMessage($message) {
        ...
        if (strtoupper(substr($message, 0, 4)) == 'HTTP') {
            ...
            return $this->createResponse(
                            $data['code'], $data['headers'], $data['body'] === '' ? null : $data['body'], $data
            );
        }
        ...
        return $this->createRequest(
                        $data['method'], Url::buildUrl($data['request_url']), [
        ...
        foreach ($options as $key => $value) {
            ...
            switch ($key) {
                ...
                case 'auth':
                    ...
                    if ($type == 'basic') {
                        $request->setHeader(
                                'Authorization', 'Basic ' . base64_encode("$value[0]:$value[1]")
                        );
                    } elseif ($type == 'digest') {
                ...
                case 'events':
                    ...
                    $this->attachListeners($request, $this->prepareListeners(
                                    $value, ['before', 'complete', 'error', 'progress', 'end']
                            )
                    );
                    break;
