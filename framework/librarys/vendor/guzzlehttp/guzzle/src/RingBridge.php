...
class RingBridge {
    ...
    Transaction $trans, array $response, MessageFactoryInterface $messageFactory, callable $fsm
    ) {
        ...
        if (!empty($response['status'])) {
            ...
            $trans->response = $messageFactory->createResponse(
                    $response['status'], isset($response['headers']) ? $response['headers'] : [], isset($response['body']) ? $response['body'] : null, $options
            );
    ...
    public static function fromRingRequest(array $request) {
        ...
        return new Request(
                $request['http_method'], Core::url($request), isset($request['headers']) ? $request['headers'] : [], isset($request['body']) ? Stream::factory($request['body']) : null, $options
        );
    }
