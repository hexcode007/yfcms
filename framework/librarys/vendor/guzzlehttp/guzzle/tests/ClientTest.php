...
class ClientTest extends \PHPUnit_Framework_TestCase {
    ...
    public function testAddsDefaultUserAgentHeaderWithDefaultOptions() {
        ...
        $this->assertFalse($client->getDefaultOption('allow_redirects'));
        $this->assertEquals(
                ['User-Agent' => Client::getDefaultUserAgent()], $client->getDefaultOption('headers')
        );
    }
    ...
    public function testDoesNotOverwriteExistingUA() {
        ...
        $this->assertEquals(
                ['User-Agent' => 'test'], $client->getDefaultOption('headers')
        );
    }
    ...
    public function testUsesBaseUrlWhenNoUrlIsSet() {
        ...
        $this->assertEquals(
                'http://www.foo.com/baz?bam=bar', $client->createRequest('GET')->getUrl()
        );
    }
    ...
    public function testUsesBaseUrlCombinedWithProvidedUrl() {
        $client = new Client(['base_url' => 'http://www.foo.com/baz?bam=bar']);
        $this->assertEquals(
                'http://www.foo.com/bar/bam', $client->createRequest('GET', 'bar/bam')->getUrl()
        );
    }
    ...
    public function testUsesBaseUrlCombinedWithProvidedUrlViaUriTemplate() {
        $client = new Client(['base_url' => 'http://www.foo.com/baz?bam=bar']);
        $this->assertEquals(
                'http://www.foo.com/bar/123', $client->createRequest('GET', ['bar/{bam}', ['bam' => '123']])->getUrl()
        );
    }
    ...
    public function testSettingAbsoluteUrlOverridesBaseUrl() {
        $client = new Client(['base_url' => 'http://www.foo.com/baz?bam=bar']);
        $this->assertEquals(
                'http://www.foo.com/foo', $client->createRequest('GET', '/foo')->getUrl()
        );
    }
    ...
    public function testSettingAbsoluteUriTemplateOverridesBaseUrl() {
        $client = new Client(['base_url' => 'http://www.foo.com/baz?bam=bar']);
        $this->assertEquals(
                'http://goo.com/1', $client->createRequest(
                        'GET', ['http://goo.com/{bar}', ['bar' => '1']]
                )->getUrl()
        );
    }
    ...
    public function testSendingRequestCanBeIntercepted() {
        ...
        $client->getEmitter()->on(
                'before', function (BeforeEvent $e) use ($response) {
    ...
    public function testClientHandlesErrorsDuringBeforeSend() {
        ...
        $this->assertEquals(
                200, $client->get('http://test.com')->getStatusCode()
        );
    }
    ...
    public function testCanInjectResponseForFutureError() {
        ...
        $future = new FutureArray(
                $deferred->promise(), function () use ($deferred, &$calledFuture) {
            $calledFuture = true;
    ...
    public function testCanReturnFutureResults() {
        ...
        $deferred = new Deferred();
        $future = new FutureArray(
                $deferred->promise(), function () use ($deferred, &$called) {
            $called = true;
    ...
    public function testUsesProxyEnvironmentVariables() {
        ...
        $client = new Client();
        $this->assertEquals(
                ['http' => '127.0.0.1'], $client->getDefaultOption('proxy')
        );

        putenv('HTTPS_PROXY=127.0.0.2');
        $client = new Client();
        $this->assertEquals(
                ['http' => '127.0.0.1', 'https' => '127.0.0.2'], $client->getDefaultOption('proxy')
        );
