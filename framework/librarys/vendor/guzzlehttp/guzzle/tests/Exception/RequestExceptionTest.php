...
class RequestExceptionTest extends \PHPUnit_Framework_TestCase {
    ...
    public function testCreatesClientErrorResponseException() {
        ...
        $this->assertEquals(
                'Client error response [url] / [status code] 400 [reason phrase] Bad Request', $e->getMessage()
        );
    ...
    public function testCreatesServerErrorResponseException() {
        ...
        $this->assertEquals(
                'Server error response [url] / [status code] 500 [reason phrase] Internal Server Error', $e->getMessage()
        );
    ...
    public function testCreatesGenericErrorResponseException() {
        ...
        $this->assertEquals(
                'Unsuccessful response [url] / [status code] 600 [reason phrase] ', $e->getMessage()
        );
        $this->assertInstanceOf('GuzzleHttp\Exception\RequestException', $e);
    }
