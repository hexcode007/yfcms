...
class FutureResponseTest extends \PHPUnit_Framework_TestCase {
    ...
    public function testDoesTheSameAsResponseWhenDereferenced() {
        ...
        $this->assertSame(
                $response->getBody(), $future->getBody()
        );
        $this->assertSame(
                $response->getProtocolVersion(), $future->getProtocolVersion()
        );
        $this->assertSame(
                $response->getEffectiveUrl(), $future->getEffectiveUrl()
        );
        $future->setEffectiveUrl('foo');
        $this->assertEquals('foo', $response->getEffectiveUrl());
        $this->assertSame(
                $response->getReasonPhrase(), $future->getReasonPhrase()
        );
