...
class PostBodyTest extends \PHPUnit_Framework_TestCase {
    ...
    public function testCanForceMultipartUploadsWhenApplying() {
        ...
        $b->applyRequestHeaders($m);
        $this->assertContains(
                'multipart/form-data', $m->getHeader('Content-Type')
        );
    }
    ...
    public function testApplyingWithFieldsAddsMultipartUpload() {
        ...
        $b->applyRequestHeaders($m);
        $this->assertContains(
                'application/x-www-form', $m->getHeader('Content-Type')
        );
        $this->assertTrue($m->hasHeader('Content-Length'));
    }
    ...
    public function testCreatesMultipartUploadWithMultiFields() {
        ...
        $this->assertContains(
                'Content-Disposition: form-data; name="third"', $s
        );
        $this->assertContains(
                'Content-Disposition: form-data; name="other"', $s
        );
    }
    ...
    public function testMultipartWithBase64Fields() {
        ...
        $b->forceMultipartUpload(true);
        $this->assertEquals(
                ['foo64' => '/xA2JhWEqPcgyLRDdir9WSRi/khpb2Lh3ooqv+5VYoc='], $b->getFields()
        );
        $m = new Request('POST', '/');
        $b->applyRequestHeaders($m);
        $this->assertContains(
        ...
        $this->assertContains(
                '/xA2JhWEqPcgyLRDdir9WSRi/khpb2Lh3ooqv+5VYoc=', $contents
        );
    }
