...
class mysqlHandler implements dbInterface {
    ...
    public function table_drop($table) {
        ...
        // 删除 count, maxid
        try {
            $this->query("DELETE FROM {$this->tablepre}framework_count WHERE name='$table'", $this->xlink);
        } catch (Exception $e) {
            
        };
        try {
            $this->query("DELETE FROM {$this->tablepre}framework_maxid WHERE name='$table'", $this->xlink);
        } catch (Exception $e) {
